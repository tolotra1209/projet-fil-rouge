...
void enregistrerPlagesB(FILE *outputFilePlagesB, struct InformationsImage *infos, const char *couleur);
void compterDansPlageB(FILE *outputFilePlagesB, struct InformationsImage *infos, const char *couleur);
...
void enregistrerPlagesB(FILE *outputFilePlagesB, struct InformationsImage *infos, const char *couleur) {
    ...
    for (int i = 0; i < infos->nbLignes; i++) {
        ...
        if (plageB.debut != -1 && plageB.fin != -1) {
            // Utilisez fprintf pour écrire dans le fichier au lieu de printf
...
// Modifiez l'appel de la fonction dans compterDansPlageB
void compterDansPlageB(FILE *outputFilePlagesB, struct InformationsImage *infos, const char *couleur) {
    char couleurI;
    ...
    for (int i = 0; i < infos->nbLignes; i++) {
        ...
        fprintf(outputFilePlagesB, "Pour la ligne %d, entre les colonnes %d et %d : ", plageB.numeroLigne, debutPlage, finPlage);
        ...
        if (debutPlage != -1 && finPlage != -1) {
            ...
            fprintf(outputFilePlagesB, "Pas de plage '%s' sur cette ligne.\n", couleur);
        }

        // Enregistrement des plages dans le fichier de sortie
        enregistrerPlagesB(outputFilePlagesB, infos, couleur);
    }
}
const char *trouverMinMaxPlagesB(struct InformationsImage *infos, const char *couleur) {
    ...
    if (plageTrouvee) {
        ...
        if (balle) {
            phrase = (strcmp(couleur, "bleu") == 0) ? "balle bleue" : ((strcmp(couleur, "rouge") == 0) ? "balle rouge" : "aucune balle reconnue");
...
const char *traiterImage(char *cheminFichier, struct InformationsImage *infos, const char *couleur) {
    ...
    FILE *outputFilePlagesB = fopen("resultatPlagesB.txt", "w");

    if (outputFilePlagesB == NULL) {
        phrase = "Impossible d'ouvrir le fichier de sortie pour les résultats des plages B.";
        fclose(fichier);
        return phrase;
    }

    // Compter les 'B' et 'N' dans chaque plage B
    compterDansPlageB(outputFilePlagesB, infos, couleur);
